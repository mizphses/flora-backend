...
export class AuthService {
  ...
  async signup(email: string, password: string, jwtSecret: string) {
    const user = await this.prisma.crewIdentity.findUnique({
      where: { email },
    });
    ...
    const newUser = await this.prisma.crewIdentity.create({
      data: {
        email,
        password: hashedPassword,
        roles: 'RSV_TKT_GENERAL',
      },
    });
  ...
  async refreshToken(refreshToken: string, jwtSecret: string) {
    ...
    return this.createTokens(token.crewIdentityId, jwtSecret);
  }
